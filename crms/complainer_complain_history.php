...
<head>
    ...
    header("location:userlogin.php");
        ...
        $result=mysqli_query($conn,"SELECT aadhar_no FROM user where user_id='$u_id'");
    ...
    if(isset($_POST['s2']))
    {
        ...
        {            
            ...
            $_SESSION['cid']=$cid;
            $cid=$_SESSION['cid'];
            $resu=mysqli_query($conn,"SELECT aadhar_no FROM complaint where complaint_id='$cid'");
            $res=mysqli_query($conn,"select * from complaint_status where complaint_id='$cid'");
    ...
    $query="select * from complaint c,complaint_status cs where c.aadhar_no='$a_no' AND cs.complaint_id = c.complaint_id order by complaint_id desc";
...
<?php
// $query = "select * from complaint_status where complaint_id='1'"
    ...
    <tbody style="background-color: white; color: black;">
      <tr>
        ...
        <td><?php echo  $rows['type_of_crime'] ; ?></td>
        <td><?php echo  $rows['date_of_crime'] ; ?></td>
        <td><?php echo  $rows['location_of_crime'] ; ?></td>
        <td><?php echo  $stat['status_of_complaint'] ; ?></td>
      </tr>
    </tbody>
    <?php
    } 
